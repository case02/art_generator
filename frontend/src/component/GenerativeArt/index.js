...
export default function GenerativeArt() {
	...
	function canvasApp() {
		...
		function init() {
            ...
            and allow the radius of them to consty. If so, modify the next three constiables.
            */
			numCircles = 1;
			maxMaxRad = 200;
			minMaxRad = 200;
			...
			/*
            We draw closed curves with constying radius. The factor below should be set between 0 and 1,
		...
		function onTimer() {
			...
			for (j = 0; j < drawsPerFrame; j++) {
				...
				for (i = 0; i < numCircles; i++) {
					...
					//It is like drawing a circle, except with constying radius.
		...
		//Here is the function that defines a noisy (but not wildly constying) data set which we will use to draw the curves.
		...
		function exportPressed(evt) {
			...
			const metrics = exportCanvasContext.measureText('rectangleworld.com');
